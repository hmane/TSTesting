...
const FormValue: React.FC<IFormValueProps> = ({ children, className = '' }) => {
  ...
  return (
    <div className={`${styles.formValue} ${className}`}>
      <div className={styles.controlContainer}>{formControls}</div>
      {descriptionAndError.length > 0 && (
        <div className={styles.descriptionErrorContainer}>{descriptionAndError}</div>
      )}
    </div>
  );
};
