...
const FormItem: React.FC<IFormItemProps> = ({ children, className = '', style, labelWidth }) => {
  ...
  const itemClassName = `${styles.formItem} ${
    labelWidth ? styles.customLabelWidth : ''
  } ${className}`;

  return (
    <div className={itemClassName} style={itemStyle}>
      {label && <div className={styles.labelArea}>{label}</div>}
      {value && <div className={styles.valueArea}>{value}</div>}
    </div>
  );
};
