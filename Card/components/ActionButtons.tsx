...
import { ActionButtonsProps, CardAction } from '../types/Card.types';
...
export const ActionButtons = memo<ActionButtonsProps>(({
  ...
  const handleMaximizeClick = useCallback((event: React.MouseEvent) => {
    event.stopPropagation();
    if (!disabled) {
  ...
  const renderActionButton = useCallback((action: CardAction) => {
    ...
    const buttonContent = (
      <ButtonComponent
        key={action.id}
        id={buttonId}
        className={`spfx-card-action-btn ${action.variant || 'secondary'} ${action.hideOnMobile ? 'desktop-only' : ''}`}
  ...
  const expandButton = useMemo(() => {
    ...
    return button;
  }, [allowExpand, hideExpandButton, isExpanded, accessibility, handleExpandClick, disabled, showTooltips]);
  ...
  const maximizeButton = useMemo(() => {
    ...
    return button;
  }, [allowMaximize, hideMaximizeButton, isMaximized, accessibility, handleMaximizeClick, disabled, showTooltips]);
