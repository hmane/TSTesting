...
export const useParentDetection = () => {
   ...
   */
  const expandParentAccordion = useCallback(
    async (accordionId: string, cardId: string): Promise<boolean> => {
      try {
  ...
  const isParentExpanded = useCallback(
    (fieldRef: RefObject<HTMLElement>): boolean => {
  ...
  const getParentInfo = useCallback(
    (fieldRef: RefObject<HTMLElement>): ParentInfo | null => {
