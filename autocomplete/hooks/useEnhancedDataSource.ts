...
export const useEnhancedDataSource = (options: UseEnhancedDataSourceOptions) => {
      ...
      try {
        ...
        if (excludeSelected && excludeSelected.length > 0) {
          results = results.filter(item => {
            ...
            return !excludeSelected.some(selectedKey => compareKeys(itemKey, selectedKey));
          });
        }
        ...
        if (
          ...
          (!loadOptions.searchValue || loadOptions.searchValue.length === 0)
        ) {
          try {
            const recentItems = cacheManagerRef.current.getRecentItemsExcluding(
              excludeSelected || []
            );
            ...
            const recentKeys = recentDataItems.map(
              item => item[loadOptions.userData?.valueField || 'id']
            );
  ...
  const customByKey = useCallback(
    async (key: any): Promise<any> => {
      try {
        let result: any = null;
